...
def get_reg_loss(tfs):
    ...
    with tf.name_scope('reg_errors'):
        ...
        if 'dwdt' in tfs.sys_para.reg_coeffs:
            ...
            reg_loss = reg_loss + dwdt_reg_alpha * tf.nn.l2_loss((new_weights[:,1:] - new_weights[:,:tfs.sys_para.steps + 3]) / tfs.sys_para.dt)
        ...
        if 'd2wdt2' in tfs.sys_para.reg_coeffs:
            ...
            reg_loss = reg_loss + d2wdt2_reg_alpha * tf.nn.l2_loss((new_weights[:,2:] - 2 * new_weights[:,1:tfs.sys_para.steps + 3] + new_weights[:,:tfs.sys_para.steps + 2]) / (tfs.sys_para.dt**2))
        ...
        if 'forbidden_coeff_list' in tfs.sys_para.reg_coeffs:
            ...
            if tfs.sys_para.is_dressed:
                v_sorted = tf.constant(c_to_r_mat(np.reshape(sort_ev(tfs.sys_para.v_c, tfs.sys_para.dressed_id), [len(tfs.sys_para.dressed_id), len(tfs.sys_para.dressed_id)])), dtype=tf.float32)
            ...
            for inter_vec in tfs.inter_vecs:
                ...
                for inter_reg_alpha_coeff, state in zip(tfs.sys_para.reg_coeffs['forbidden_coeff_list'], tfs.sys_para.reg_coeffs['states_forbidden_list']):
                    ...
                    forbidden_state_pop = tf.square(inter_vec[state,:]) + tf.square(inter_vec[tfs.sys_para.state_num + state,:])
